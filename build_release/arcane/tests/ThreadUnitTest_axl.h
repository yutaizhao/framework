...
#ifndef ARCANE_CASEOPTION_AXLSTAR_ARCANETEST_THREADUNITTEST_H
#define ARCANE_CASEOPTION_AXLSTAR_ARCANETEST_THREADUNITTEST_H
...
//! Options
class CaseOptionsThreadUnitTest
{
public:
  ...
  CaseOptionsThreadUnitTest(Arcane::ICaseOptions* co)
    : m_case_options(co)
  {
...
#ifndef ARCANE_MODULE_AXLSTAR_ARCANETEST_THREADUNITTEST_H
#define ARCANE_MODULE_AXLSTAR_ARCANETEST_THREADUNITTEST_H
...
class ArcaneThreadUnitTestObject
...
public:
  ...
  {
    si->setCaseOptionsFileName("ThreadUnitTest_arcane_tests");
    si->setAxlVersion(1.0);
    si->setDefaultTagName("thread-unit-test");
...
public:
  ...
  //! Constructeur
  ArcaneThreadUnitTestObject(const Arcane::ServiceBuildInfo& sbi)
    : Arcane::BasicUnitTest(sbi)
    , m_options(nullptr)
  {
    Arcane::ICaseOptions* co = sbi.caseOptions();
    if (co) {
      m_options = new CaseOptionsThreadUnitTest(co);
    }
	
  }

  //! Destructeur
  virtual ~ArcaneThreadUnitTestObject()
  {
    delete m_options;
  }
...
public:
  ...
  CaseOptionsThreadUnitTest* options() const { return m_options; }
 
private:
        
  //! Options du jeu de données du service
  CaseOptionsThreadUnitTest* m_options;
...
#define ARCANE_REGISTER_SERVICE_THREADUNITTEST(service_name,class_name) \
