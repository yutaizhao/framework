...
#ifndef ARCANE_CASEOPTION_AXLSTAR_ARCANETEST_MATVECUNITTEST_H
#define ARCANE_CASEOPTION_AXLSTAR_ARCANETEST_MATVECUNITTEST_H
...
//! Options
class CaseOptionsMatVecUnitTest
{
public:
  ...
  CaseOptionsMatVecUnitTest(Arcane::ICaseOptions* co)
    : m_case_options(co)
  {
...
#ifndef ARCANE_MODULE_AXLSTAR_ARCANETEST_MATVECUNITTEST_H
#define ARCANE_MODULE_AXLSTAR_ARCANETEST_MATVECUNITTEST_H
...
class ArcaneMatVecUnitTestObject
...
public:
  ...
  {
    si->setCaseOptionsFileName("MatVecUnitTest_arcane_tests");
    si->setAxlVersion(1.0);
    si->setDefaultTagName("mat-vec-unit-test");
...
public:
  ...
  //! Constructeur
  ArcaneMatVecUnitTestObject(const Arcane::ServiceBuildInfo& sbi)
    : Arcane::BasicUnitTest(sbi)
    , m_options(nullptr)
  {
    Arcane::ICaseOptions* co = sbi.caseOptions();
    if (co) {
      m_options = new CaseOptionsMatVecUnitTest(co);
    }
	
  }

  //! Destructeur
  virtual ~ArcaneMatVecUnitTestObject()
  {
    delete m_options;
  }
...
public:
  ...
  CaseOptionsMatVecUnitTest* options() const { return m_options; }
 
private:
        
  //! Options du jeu de données du service
  CaseOptionsMatVecUnitTest* m_options;
...
#define ARCANE_REGISTER_SERVICE_MATVECUNITTEST(service_name,class_name) \
