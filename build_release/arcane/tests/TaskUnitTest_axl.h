...
#ifndef ARCANE_CASEOPTION_AXLSTAR_ARCANETEST_TASKUNITTEST_H
#define ARCANE_CASEOPTION_AXLSTAR_ARCANETEST_TASKUNITTEST_H
...
public:
  ...
  CaseOptionsTaskUnitTest(Arcane::ICaseOptions* co)
    : m_case_options(co)
  {
...
#ifndef ARCANE_MODULE_AXLSTAR_ARCANETEST_TASKUNITTEST_H
#define ARCANE_MODULE_AXLSTAR_ARCANETEST_TASKUNITTEST_H
...
class ArcaneTaskUnitTestObject
...
public:
  ...
  {
    si->setCaseOptionsFileName("TaskUnitTest_arcane_tests");
    si->setAxlVersion(1.0);
    si->setDefaultTagName("task-unit-test");
...
public:
  ...
  //! Constructeur
  ArcaneTaskUnitTestObject(const Arcane::ServiceBuildInfo& sbi)
    : Arcane::BasicUnitTest(sbi)
    , m_options(nullptr)
  {
    Arcane::ICaseOptions* co = sbi.caseOptions();
    if (co) {
      m_options = new CaseOptionsTaskUnitTest(co);
    }
	
  }

  //! Destructeur
  virtual ~ArcaneTaskUnitTestObject()
  {
    delete m_options;
  }
...
public:
  ...
  CaseOptionsTaskUnitTest* options() const { return m_options; }
 
private:
        
  //! Options du jeu de données du service
  CaseOptionsTaskUnitTest* m_options;
...
#define ARCANE_REGISTER_SERVICE_TASKUNITTEST(service_name,class_name) \
