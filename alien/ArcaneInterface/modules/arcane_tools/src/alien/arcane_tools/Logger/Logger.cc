...
namespace Alien {
  ...
  stop(eStep step)
  {
    ...
    m_solver_infos.m_init_time = m_parallel_mng->reduce(Parallel::ReduceMax, m_solver_infos.m_init_time);
  }
  ...
  stop(eStep step, const SolverStatus& stat)
  {
    ...
    time = m_parallel_mng->reduce(Parallel::ReduceMax, time);
    SolverStatus status;
  ...
  package() const
  {
    return m_values.at("package").back();
  }
  ...
  maxIt() const
  {
    return m_values.at("max-it").back();
  }
  ...
  solver() const
  {
    return m_values.at("solver").back();
  }
  ...
  precond() const
  {
    return m_values.at("precond").back();
  }
